...
RSpec.describe CheckByOwnerAddressWorker do
  describe '#perform' do
    ...
    before do
      allow(Positions::CheckByOwnerAddress).to receive(:new)
        .with(address, threshold).and_return(check_service_double)
    end
    ...
    context 'when there are users with telegram_chat_id', testing: :inline do
      it 'asynchronously checks every user with telegram_chat_id' do
        perform_worker
        expect(check_service_double).to have_received(:call).once
      end
    end
  end
end
